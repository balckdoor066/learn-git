...
function PixelDesk({ agent, selected, onSelect }: { agent: Agent; selected: boolean; onSelect: (agent: Agent) => void }) {
  ...
  return (
    ...
    >
      <span className="speech-dot"><MessageSquare size={14} /></span>
...
function OfficeMap({ agents, selected, onSelect }: { agents: Agent[]; selected: Agent; onSelect: (agent: Agent) => void }) {
  return (
    <section className="office-map-panel">
      ...
      <div className="room room-comms"><span>COMMS DESK</span><i /></div>
      <div className="room room-code"><span>CODE LAB</span><i /></div>
      <div className="room room-design"><span>DESIGN STUDIO</span><i /></div>
      <div className="room room-server"><span>SERVER ROOM</span><i /></div>
      <button className="meeting-room" onClick={() => onSelect(agents[0])}>
        <span>MEETING ROOM</span>
        <b />
      </button>
      <div className="room room-research"><span>RESEARCH LIBRARY</span><i /></div>
      <div className="room room-review"><span>REVIEW CORNER</span><i /></div>
      <div className="room room-strategy"><span>STRATEGY ROOM</span><i /></div>
...
function StatsPanels({ agents }: { agents: Agent[] }) {
  ...
  return (
    <>
      ...
      <section className="bottom-panel system-panel"><h3>System Status</h3>{['Hermes API','WebSocket','Database','Queue Workers'].map((label, index) => <p key={label}><span>{label}</span><b>{index === 3 ? '5 / 5' : index === 1 ? 'CONNECTED' : index === 2 ? 'HEALTHY' : 'ONLINE'}</b></p>)}<div className="radar-chart"><i /></div></section>
    </>
  );
}
...
export default function App() {
  ...
  return (
    <main className="command-center">
      ...
      <div className="version-line">HERMES COMMAND CENTER v0.2.0</div>
    </main>
  );
}
